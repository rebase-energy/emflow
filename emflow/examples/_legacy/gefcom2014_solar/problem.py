...
with importlib.resources.path("emflow.examples.data", "gefcom2014-solar.csv") as path:
    df_data = pd.read_csv(path, index_col=[0, 1], parse_dates=True, header=[0, 1])
with importlib.resources.path("emflow.examples.data", "gefcom2014-solar-scores.csv") as path:
...
class GEFCom2014SolarEnv(gym.Env):
    def __init__(self, dataset: ef.Dataset, state_space: ef.StateSpace, action_space: ef.ActionSpace): 
    ...
    def reset(self):
        ...
        initial_input = initial_dataframe.loc[:,(slice(None), self.input)]
        initial_target = initial_dataframe.loc[:,(slice(None), self.target)]
...
def get_problem_simple():
    ...
    env = GEFCom2014SolarEnv(dataset=dataset, state_space=state_space, action_space=action_space)

    from emflow.problems.objective import PinballLoss
    obj = PinballLoss(quantiles=[0.1, 0.5, 0.9])

    problem = ef.Problem(name="gefcom2014-solar:baseline", 
                      environment=env,
                      objective=obj)
